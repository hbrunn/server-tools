...
class Letsencrypt(models.AbstractModel):
        ...
        for authorizations in authzr.authorizations:
            for challenge in sorted(
                ...
                if challenge.chall.typ == TYPE_CHALLENGE_HTTP:
                    ...
                    self._respond_challenge_dns(challenge, domain)
                    client.answer_challenge(
    ...
    def _cascade_domains(self, altnames):
        if len(altnames) > 1:
            for altname in filter(lambda x: '*.' in x, altnames):
                def _find_wildcard_index(altname):
                    for index, item in enumerate(altname.split('.')):
                        if '*' in item:
                            return index
                w_index = _find_wildcard_index(altname)
                # get the location of the asterisk
                # for domains that are the same after the asterisk
                # and have the same levels, remove them
                # except the wildcard one
                to_delete = filter(
                    lambda x: len(x.split('.')) == len(altname.split('.'))
                    and x.split('.')[w_index + 1:] == altname.split('.')[
                        w_index + 1:]
                    and '*' not in x,
                    altnames,
                )
                for to_delete_item in to_delete:
                    altnames.remove(to_delete_item)
        return altnames
    ...
    def _make_csr(self, account_key, domain_key_file, domain):
        ...
        if altnames:
            ...
            self._cascade_domains(valid_domains)
        else:
